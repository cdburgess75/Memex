...
// The row itself, and its owner's grant, on the client of the placement that holds the
// library's tree lock. Every route that places a document by name writes it through
// here, so there is one INSERT to keep right.
async function insertDocumentRow(q, { name, size, mimetype, storagePath, user, documentText = null, libraryId, contentHash = null, libraryScoped = false }) {
  const doc = await q.queryOne(
    ...
    [name, size || 0, mimetype, storagePath, user.id, user.email, documentText, libraryId, contentHash, !!libraryScoped]
  );
...
async function createDocumentRecord({ displayName, storagePath, mimetype, storedSize, user, sourceDetail, libraryId, notifyUpload = false, libraryScoped = false, resolve = null }) {
  ...
  const placed = await folderOps().withFolderOp({ libraryIds: [lib], kind: 'placement', mode: 'shared' }, async (q) => {
    ...
    const doc = await insertDocumentRow(q, { name, size: storedSize, mimetype, storagePath, user, documentText, libraryId: lib, contentHash, libraryScoped: scoped });
    return { doc, name, deduped: false };
  }).catch(async (e) => {
    // Nothing was placed -- refused where it turned out to land, or the library was busy
    // for longer than a placement waits -- so no row points at the blob the caller
    // stored for it. Drop it here, once, rather than in every caller's catch.
