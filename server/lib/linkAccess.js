...
// The creator's live account, if it may publish links at all; null otherwise.
async function linkCreator(createdBy) {
...
async function servableDocs(createdBy, ids, columns = 'd.id') {
  const creator = await linkCreator(createdBy);
