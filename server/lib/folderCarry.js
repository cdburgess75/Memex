...
// End the live folder links at and under a path. Called wherever the folder stops being
// there: deleted (endShares, below), or moved to ANOTHER library -- which hands its content
// to that library's owner and ends every share of it, and a link somebody sent from the old
// library is a share of it. Returns how many ended.
async function endLiveLinks(q, { libraryId, path, user }) {
  if (!path) return 0;
...
async function endShares(q, { libraryId, path, opId, user, cause }) {
  ...
  await endLiveLinks(q, { libraryId, path, user });
  return q.query(
    `WITH gone AS (
...
module.exports = { endLiveLinks, maxRows, grantsUnder, countUnder, refuseIfTooBig, planSharePaths, assertTotal, carryPathState, carryNotifyPrefs, endShares, recordOpDocuments, recordOp };
