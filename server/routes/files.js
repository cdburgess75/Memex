...
// Shared file-domain helpers, split out of this router (ST-1). Imported back under
// their original names so every call site in this file is unchanged, and the module
// re-exports at the bottom stay stable for existing consumers.
const { logEvent, logDocumentEvent, requestAuditDetail } = require('../lib/fileEvents');
const {
  tokenHash, passwordParts, verifySharePassword, shareLinkClientShape,
  issueShareTicket, verifyShareTicket, publicAppBase,
...
router.get('/share/:token/info', async (req, res) => {
  try {
    ...
    // Seeing the file's page IS opening the share -- but only once it shows anything: a
    // password page that was never unlocked has told nobody anything.
    if (unlocked) await shareOpens.linkOpened(share);
    res.json({
...
router.get('/signin-link/:token', auth, async (req, res) => {
  try {
    ...
    if (!me || me !== String(share.recipient_email || '').toLowerCase()) {
      return res.status(403).json({ error: `This link was sent to ${share.recipient_email}. Sign in as that person to open it.` });
    }
    ...
    res.json({
      ...
      // to disk instead of being buffered in the page.
      downloadUrl: `/api/files/share/${encodeURIComponent(req.params.token)}?dl=${encodeURIComponent(issueShareTicket(share.id))}`,
    });
  } catch (e) { serverError(res, e); }
});
...
router.post('/upload', auth, requireRole('admin', 'contributor'), (req, res, next) => getUpload().then(mw => mw(req, res, next)).catch(next), async (req, res) => {
  ...
  try {
    ...
    const { doc, name } = await withFolderOp({ libraryIds: [uploadLibraryId], kind: 'placement', mode: 'shared' }, async (q) => {
      const landed = await landingName(req.body.displayName || originalname, uploadLibraryId, req.user, q);
      if (landed === null) throw new FolderOpError(null, 400, 'invalid folder');
      const at = landed || 'upload';
      const right = await libraries.writeRight(req.user, uploadLibraryId, parentOf(at), q);
      ...
      const row = await q.queryOne(
        ...
        [at, size, mimetype, storagePath, req.user.id, req.user.email, documentText, uploadLibraryId, right.scoped]
      );
      await documentAccess.grantOwnerAdmin(row.id, req.user, q);
      return { doc: row, name: at };
    });
...
router.post('/upload-stream', auth, requireRole('admin', 'contributor'), async (req, res) => {
  ...
  try {
    ...
    const { doc, name } = await withFolderOp({ libraryIds: [uploadLibraryId], kind: 'placement', mode: 'shared' }, async (q) => {
      const landed = await landingName(rawName, uploadLibraryId, req.user, q);
      if (landed === null) throw new FolderOpError(null, 400, 'invalid folder');
      const at = landed || 'upload';
      const right = await libraries.writeRight(req.user, uploadLibraryId, parentOf(at), q);
      if (right.status) throw new FolderOpError(null, right.status, right.error);
      const row = await q.queryOne(
        `INSERT INTO documents (name, size, mime_type, storage_path, uploaded_by, uploaded_by_email, document_text, library_id, library_scoped)
         VALUES ($1, $2, $3, $4, $5, $6, $7, $8, $9) RETURNING ${DOCUMENT_COLUMNS}`,
        [at, storedSize || 0, mimetype, storagePath, req.user.id, req.user.email, documentText, uploadLibraryId, right.scoped]
      );
      await documentAccess.grantOwnerAdmin(row.id, req.user, q);
      return { doc: row, name: at };
    });
    ...
    const { doc, canIngest } = await createDocumentRecord({
      ...
      resolve: async (q) => {
        ...
        if (landingPath !== session.name) {
          const moved = await libraries.writeRight(req.user, completeLibraryId, parentOf(landingPath), q);
          if (moved.status) {
        ...
        return { displayName: landingPath, libraryScoped: dest.scoped };
      },
    });
    ...
    for (const to of recipients) {
      try {
        ...
        if (isInternal(to)) {
          ...
          if (to !== sender) {
            notifications.create({ userEmail: to, type: 'share_granted', title: `${senderName} sent you a file`, body: `"${doc.name}" · view`, refType: 'signin_link', refId: null, refPath: token }).catch(() => {});
          }
          const mail = await email.sendMail({
          ...
          results.push({ to, kind: 'signin_link', sent: mail?.sent !== false, reason: mail?.sent === false ? mail.reason : undefined });
          continue;
        }
...
router.post('/:id/restore-version/:versionId', auth, requireRole('admin', 'contributor'), async (req, res) => {
  try {
    ...
    const updated = await withFolderOp({ libraryIds: [doc.library_id], kind: 'placement', mode: 'shared' }, async (q) => {
      const now = await q.queryOne('SELECT name FROM documents WHERE id = $1', [doc.id]);
  ...
  try {
    ...
    const { doc } = await createDocumentRecord({
      ...
      resolve: async (q) => {
        ...
        const right = await libraries.writeRight(req.user, libraryId, parentOf(nameNow), q);
        if (right.status) throw new FolderOpError(null, right.status, right.error);
...
router.post('/:id/restore', auth, requireRole('admin', 'contributor'), async (req, res) => {
  try {
    ...
    const back = await withFolderOp({ libraryIds: [doc.library_id], kind: 'placement', mode: 'shared' }, async (q) => q.queryOne(
      `UPDATE documents
