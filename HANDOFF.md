...
_Last updated: 2026-06-04 · Running version: **v2026.06.04.002**_
...
- **Branch:** `claude/url-request-GwwHe`. **Origin tip is `98ddcfe`** (confirmed via GitHub API).
  ...
  pushed. **6 commits after `98ddcfe` are LOCAL-only on the box** and need pushing.
...
- Local-only commits: `RECOMMENDATIONS.md`; Caddy TLS overlay; trash/audit/perms;
  `v2026.06.04.001` version scheme; `v2026.06.04.002` Trash UI; this handoff update.
- Local-only tags: `v2026.06.04.001`, `v2026.06.04.002`.
...
3. **Next Phase 1 build items** (each its own version bump):
   - `.003` Rate limiting (`express-rate-limit`; add dep to `server/package.json`) — prerequisite for any public route.
...
## How to resume
SSH: `sshpass -p '<root pw>' ssh root@192.168.1.32`. Code edits: edit under `/opt/memex`,
