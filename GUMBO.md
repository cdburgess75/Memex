# GUMBO

Open source software used in this project. Thank you to every author and contributor.

---

## Frontend

### Supabase JS Client
- **Package:** `@supabase/supabase-js`
- **License:** Apache License 2.0
- **Source:** https://github.com/supabase/supabase-js

### Fraunces
- **Type:** Variable display font (Google Fonts)
- **License:** SIL Open Font License 1.1
- **Source:** https://github.com/undercasetype/Fraunces
- **Authors:** Phaedra Charles, Flavia Zimbardi, Owen Earl

### Newsreader
- **Type:** Variable serif font (Google Fonts)
- **License:** SIL Open Font License 1.1
- **Source:** https://github.com/productiontype/Newsreader

### JetBrains Mono
- **Type:** Monospace font (Google Fonts)
- **License:** SIL Open Font License 1.1
- **Source:** https://github.com/JetBrains/JetBrainsMono

---

## Server

### Express
- **Package:** `express`
- **License:** MIT
- **Source:** https://github.com/expressjs/express

### Supabase JS Client
- **Package:** `@supabase/supabase-js`
- **License:** Apache License 2.0
- **Source:** https://github.com/supabase/supabase-js

### Anthropic Node SDK
- **Package:** `@anthropic-ai/sdk`
- **License:** MIT
- **Source:** https://github.com/anthropics/anthropic-sdk-node

### cors
- **Package:** `cors`
- **License:** MIT
- **Source:** https://github.com/expressjs/cors

### dotenv
- **Package:** `dotenv`
- **License:** BSD-2-Clause
- **Source:** https://github.com/motdotla/dotenv

### Cheerio
- **Package:** `cheerio`
- **License:** MIT
- **Source:** https://github.com/cheeriojs/cheerio

### Multer
- **Package:** `multer`
- **License:** MIT
- **Source:** https://github.com/expressjs/multer

### pdf-parse
- **Package:** `pdf-parse`
- **License:** MIT
- **Source:** https://gitlab.com/autokent/pdf-parse

---

## Infrastructure

### Node.js
- **License:** MIT
- **Source:** https://github.com/nodejs/node

### Docker
- **License:** Apache License 2.0
- **Source:** https://github.com/moby/moby

### Supabase (platform)
- **License:** Apache License 2.0
- **Source:** https://github.com/supabase/supabase
